...
public:
	...
	int encontrarIndice(const T& ele);
    ...
    int rutaMinima(const T &ori, const T &des); // Encuentra la ruta mínima entre dos vertices
    bool esFuertementeConexo();
    void obtenerVerticesPozos();
    void obtenerVerticesFuentes();
};
